...
当前 fork 仍将 UU 远程锁定为 `4.33.0.8907`，并兼容 x86-64 Ubuntu
22.04（GNOME 42）与 Ubuntu 24.04（GNOME 46）。24.04 仍是上游完整验证基线；
22.04 增加了 Jammy 专用的软件包、旧版 `grdctl` 与 GNOME Remote Desktop
兼容逻辑，建议先在目标主机完成一次人工安装验证后再启用无人值守。安装器会在
22.04 选择 `freerdp2-x11`、在 24.04 选择 `freerdp3-x11`，并按运行时能力
决定是否启用 `libei` 修复。Wine 仍使用 WineHQ stable。任何未知二进制文件
都会被拒绝，绝不会直接套用旧补丁。
